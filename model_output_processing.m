...
lat = nc_varget(ua_file,'lat');
lon = nc_varget(ua_file,'lon');
time = nc_varget(ua_file,'time'); % Assumes both files use the same time
ua = nc_varget(ua_file,'ua');
sat = ncread(sat_file,'ts');
precip = nc_varget(pr_file,'pr');
slp = nc_varget(slp_file,'psl');
...
for i = 1:90
    for j = 1:144
       %precip_detr(:,i,j) = detrend(precip(:,i,j),'linear');
