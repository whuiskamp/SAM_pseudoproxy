...
spot_wind = zeros(1000,499);
spot_precip = zeros(1000,499);
...
% for n=33:499
%     [pr_muhat pr_sighat] = normfit(spotz_precip(:,n));
%     [ts_muhat ts_sighat] = normfit(spotz_wind(:,n));
%     if kstest2(spotz_precip(:,n),normrnd(pr_muhat,pr_sighat,1,10000))
%         disp(['Warning: Normal Series fit (pr) is inappropriate at n=',num2str(n)]);
%     endcd
%     if kstest2(spotz_wind(:,n),normrnd(ts_muhat,ts_sighat,1,10000))
%         disp(['Warning: Normal Series fit (ts) is inappropriate at n=',num2str(n)]);
%     end    
% end

% subplot(2,1,1)
% pr_pc_spot = prctile(spot_precip,[2.5,97.5]);
% ts_pc_spot = prctile(spot_wind,[2.5,97.5]);
...
% hold on;
% plot(squeeze(spotz_precip(1,:)),'r')
% plot(squeeze(spotz_precip(2,:)),'y')
% plot(squeeze(spotz_precip(3,:)),'b')
% plot(squeeze(spotz_precip(4,:)),'g')
% plot(squeeze(spotz_precip(5,:)),'m')
% hold off
